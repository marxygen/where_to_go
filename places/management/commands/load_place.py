...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        contents.update({'latitude': coordinates['lat'], 'longitude': coordinates['lng']})
        ...
        for index, image_url in enumerate(images):
            ...
            PlaceImage.objects.get_or_create(title=place.title, order=index, place=place, image=ContentFile(requests.get(image_url).content, name=image_url.split('/')[-1]))
